defmodule QRCode.Params do
  defstruct align_coords: nil,
    block_defs: nil,
    data: '',
    dimension: nil,
    ec_level: nil,
  ...
  @version_info_poly 7973 # 16#1f25 -> 0001 1111 0010 0101
  @format_info_poly 1335 # 16#0537 -> 0000 0101 0011 1110
  @format_info_mask 21522 # 16#5412 -> 0101 0100 0001 0010

  def choose_qr_params(bin, ecc) do
    mode = choose_encoding(bin)
    {mode, version, ecc_block_defs, remainder} = choose_version(mode, ecc, byte_size(bin))
    alignment_coords = alignment_patterns(version)
    dim = QRCode.Matrix.dimension(version)
    %Params{
      mode: mode,
      version: version,
      dimension: dim,
      ec_level: ecc,
      block_defs: ecc_block_defs,
      align_coords: alignment_coords,
      remainder: remainder,
      data: bin
    }
  end

  def choose_encoding(_bin), do: :byte

  def choose_version(type, ecc, length), do:
    choose_version(type, ecc, length, @tables)

  def choose_version(:byte, ecc, length, [{{ecc, version}, {_, _, capacity, _}, ecc_blocks, remainder} | _])
    when capacity >= length, do:
      {:byte, version, ecc_blocks, remainder}
  def choose_version(type, ecc, length, [_ | t]), do:
    choose_version(type, ecc, length, t)

  def version_info_bits(%Params{version: version}) when version < 7, do: <<>>
  def version_info_bits(%Params{version: version}) when version <= 40 do
    bch = QRCode.ReedSolomon.bch_code(version, @version_info_poly)
    <<version :: size(6), bch :: size(12) >>
  end

  def format_info_bits(%Params{ec_level: ec_level, mask: mask}) do
    info = Bitwise.bsl(ecc(ec_level), 3) |> Bitwise.bor(mask)
    bch = QRCode.ReedSolomon.bch_code(info, @format_info_poly)
    info_with_ec = Bitwise.bsl(info, 10) |> Bitwise.bor(bch)
    value = Bitwise.bxor(info_with_ec, @format_info_mask)
    <<value :: size(15)>>
  end
  ...
  defp alignment_patterns(version) do
    d = dimension(version)
    l = elem(@alignment_coordinates, version - 1)
    l = for x <- l, y <- l, do: {x, y}
    l
      |> Enum.filter(fn {x, y} -> is_finder_region(d, x, y) == false end)
      |> Enum.sort(fn
        ({_, y}, {_, y0}) when y < y0 -> true
        ({x, y}, {x0, y0}) when y == y0 and x <= x0 -> true
        (_, _) -> false
      end)
  end

  defp is_finder_region(_d, x, y) when (x <= 8 and y <= 8), do: true
  defp is_finder_region(d, x, y) when (x <= 8 and y >= d), do: true
  defp is_finder_region(d, x, y) when (x >= d and y <= 8), do: true
  defp is_finder_region(_, _, _), do: false
